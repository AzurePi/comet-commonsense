    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if group['max_grad_norm'] > 0:
                clip_grad_norm_([p for p in group['params'] if p.grad is not None],
                                group['max_grad_norm'])
            ...
            for p in group['params']:
                ...
                denom = exp_avg_sq.sqrt().add(group['e'])
                ...
                step_size = lr_scheduled * (bias_correction2 ** 0.5) / bias_correction1

                with torch.no_grad():
                    p.addcdiv_(exp_avg, denom, value=-step_size)

                # Add weight decay at the end (fixed version)
                if (p.ndim > 1 or group['vector_l2']) and group['l2'] > 0:
                    with torch.no_grad():
                        p.add_(p, alpha=-lr_scheduled * group['l2'])
